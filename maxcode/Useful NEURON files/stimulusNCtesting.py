...
def runSimulation(numSegs, tau, refrac, simTime, numNeurons, path, strength, dendrites, connections, NCstrengthStim):
	"""
	...
	stims = []
	for i in range(0,numNeurons):
		stims.append(makeStimulus(Soma[i],simTime,NCstrengthStim))
	...
	file = open(path + 'spikes' + str(1) + 'strength' + str(NCstrengthStim) + '.txt',"wb")
	print path + 'spikes' + str(1) + 'strength' + str(NCstrengthStim) + '.txt'
...
numNeurons = 125
numNets = 5	
strengths = linspace(50,100,11)
op = '/home/hendemd/Desktop/MATLAB/bin/Neural research/Single network 1/'
start_time = time.time()
dendrites =[]
...
for NCstrengthStim in strengths:				
	runSimulation(numSegs, tau, refrac, simTime, numNeurons, op, 0, dendrites, connections, NCstrengthStim)
