...
def runSimulation(numSegs, tau, refrac, simTime, numNeurons, path, current, strength, j):
	"""
	...
	file = open(path + 'spikes' + str(current) + 'strength' + str(strength) + 'number' + str(j) + '.txt',"wb")
...
numNeurons = 118	
strengths = [50, 50, 50]
op = '/home/hendemd/Desktop/MATLAB/bin/Neural research/final cruz/'
for i in range(1, 4):
	if i == 1:
		dir1 = 'Microcolumn'
	if i == 2:
	...
	path = op + dir1 + '/' + 'Exponential neighbors/'
	dendrites =[]
	...
	connections =[]
	fh = open(path + '/Connections' + str(1) + '.txt')
	for line in fh.readlines():
		y = [value for value in line.split()]
	...
	for strength in strengths:
		runSimulation(numSegs, tau, refrac, simTime, numNeurons, path, 1, strength)
	"""
	# For nearest neighbor connections...
	path = op + dir1 + '/' + 'Nearest neighbors/'
	dendrites =[]
	fh = open(path + '/Dendrites.txt')
	for line in fh.readlines():
	    y = [value for value in line.split()]
	...
	#for strength in strengths:
	for j in range(1,4):
		runSimulation(numSegs, tau, refrac, simTime, numNeurons, path, 1, 50, j)
	"""
