...
def recordData( numNeurons, Soma, Dend, Axon):
	"""
	...
	for var in range(0,len(Dend[0])):
		len(Dend[0])
    		vec[str(var + 1)] = h.Vector()
    		vec[str(var + 1)].record(Dend[0][var](0.5)._ref_v)
	vec['soma'] = h.Vector()
    	vec['soma'].record(Soma[0](0.5)._ref_v)
	vec['axon'] = h.Vector()
	vec['axon'].record(Axon[0](1)._ref_v)
	#Record time.
	vec['t '] = h.Vector()
...
def runSimulation(numSegs, tau, refrac, simTime, numNeurons, path, strength, dendrites, connections):
	...
	vec = recordData( numNeurons, Soma, Dend, Axon )
...
numNets = 5	
strengths = [0.1]
op = '/home/hendemd/Desktop/MATLAB/bin/Paper 2/testing neural firings/'
start_time = time.time()
dendrites =[]
