...
def constructConnections( connections, dendrites, numNeurons, Soma, Axon, Dendrites, numConnections ):
	"""
	...
	for i in range(0, 1):
		...
		print numConnections
 		for j in range(1, numConnections + 1):	
			syn1 = h.ExpSyn (0.5, sec = Dendrites[0][0])	
			Axon[j].push()
			nc1 = h.NetCon( Axon[j](0.5)._ref_v, syn1 )				
			nc1.weight[0] = 1				
			SYN.append(syn1)					
			NC.append(nc1)		
			count = count + 1
...
def recordData(Soma, Axon):
	...
	vec = {}
	vec['v soma'] = h.Vector()
	vec['v soma'].record(Soma[0](0.5)._ref_v)
	vec['v axon'] = h.Vector()
	vec['v axon'].record(Axon[0](0.5)._ref_v)
	vec['t '] = h.Vector()
...
def runSimulation(numSegs, tau, refrac, simTime, numNeurons, path, strength, dendrites, connections):
	...
	plot(vec['t '],vec['v soma'], vec['t '], vec['v axon'])
...
op = '/home/hendemd/Desktop/MATLAB/bin/Neural research/data test 2 near/'
start_time = time.time()
dendrites =[]
